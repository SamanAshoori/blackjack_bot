#The losing agent - ONLY ONE DECK BRO
import numpy as np
...
class CardCountingAgent:
    ...
    def __init__(self, learning_rate=0.1, gamma=0.95, epsilon=1.0):
        ...
        self.epsilon = epsilon
        # Slower decay because our state space is larger (buckets)
        self.epsilon_decay = 0.999998 # Slower decay for 3M episodes
        self.min_epsilon = 0.01
...
def train(episodes=3_000_000):
    ...
    for i in range(episodes):
        # Two-Phase Training:
        # Phase 1 (First 50%): Flat bet (1 unit) to learn Basic Strategy without variance
        # Phase 2 (Last 50%): Spread bet (1-10 units) to learn Betting Strategy
        if i < episodes // 2:
            bet = 1
        else:
            bet = agent.get_bet_size(last_true_count)
        ...
        # Check for shuffle (deck reset)
        if info['cards_remaining'] > 45 and i >= episodes // 2:
            # If deck shuffled, reset bet to 1 (unless we are in Phase 1, where it's always 1)
            bet = 1 
            start_of_hand_tc = 0.0
        else:
            start_of_hand_tc = info['true_count']
...
def test_basic_situations(agent):
    """Test if agent learned basic strategy"""
    agent.epsilon = 0.0  # No exploration
    
    print("\n=== Testing Basic Strategy ===")
    
    # Test case 1: Player 20 vs Dealer anything → should ALWAYS stick
    print("Player 20 vs Dealer 10, TC=0:")
    state = (20, 10, 0, 2)  # bucket 2 = neutral
    action = agent.get_action(state)
    print(f"  Action: {'STICK' if action == 0 else 'HIT'} (should be STICK)")
    print(f"  Q(stick)={agent.q_table[(state, 0)]:.3f}, Q(hit)={agent.q_table[(state, 1)]:.3f}")
    
    # Test case 2: Player 12 vs Dealer 6 → should STICK
    print("\nPlayer 12 vs Dealer 6, TC=0:")
    state = (12, 6, 0, 2)
    action = agent.get_action(state)
    print(f"  Action: {'STICK' if action == 0 else 'HIT'} (should be STICK)")
    print(f"  Q(stick)={agent.q_table[(state, 0)]:.3f}, Q(hit)={agent.q_table[(state, 1)]:.3f}")
    
    # Test case 3: Player 16 vs Dealer 10 → should HIT (basic strategy)
    print("\nPlayer 16 vs Dealer 10, TC=0:")
    state = (16, 10, 0, 2)
    action = agent.get_action(state)
    print(f"  Action: {'STICK' if action == 0 else 'HIT'} (should be HIT)")
    print(f"  Q(stick)={agent.q_table[(state, 0)]:.3f}, Q(hit)={agent.q_table[(state, 1)]:.3f}")
    
    # Test case 4: Player 11 vs anything → should ALWAYS hit
    print("\nPlayer 11 vs Dealer 7, TC=0:")
    state = (11, 7, 0, 2)
    action = agent.get_action(state)
    print(f"  Action: {'STICK' if action == 0 else 'HIT'} (should be HIT)")
    print(f"  Q(stick)={agent.q_table[(state, 0)]:.3f}, Q(hit)={agent.q_table[(state, 1)]:.3f}")
...
if __name__ == "__main__":
    train()
    agent = CardCountingAgent()
    agent.load("card_counting_model.pkl")
    test_basic_situations(agent)
